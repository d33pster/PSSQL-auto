...
def readFile(current_filecount) -> int:
    ...
    for itemz in Filenames_lo:
        ...
        df_def = spark.read.format('csv').option('inferSchema', True).option('header', True).load(itemz["Location"])
        time.sleep(1)
        
        #The code trims and removes all whitespaces from every single column in your Dataframe.
        print(red("Fixing Bad Columns if any ...")) 
        tempList = [] #Edit01
        ...
        df_def = df_def.na.drop()
        time.sleep(1)
        temp = {"Name": itemz["Name"], "df": df_def}
